...
import cn.kuailaimei.store.StoreApplication;
...
public abstract class PhotoPickingFragment extends BaseFragment {
			...
			if (requestCode == TAKE_PHOTO) {
				...
				if (photoTookedPath != null && photoTookedPath.exists()) {
					if (effecType == EFFECT_TYPE_CUT) {
						...
						String picturePath = ImageUtils.compressNoLargePhoto(getActivity(), photoTookedPath,
	...
	private void initDialogMenu(String title) {
		...
		sheet.addCancelButton(R.string.cancel);
		sheet.show();
	}
	...
	private File getCompressedPhotoPath() {
		String userId = StoreApplication.getInstance().getUser().getMobile();
		if (userId == null) {
			userId = "StorePhoto_";
		} else {
			userId = (userId + "_");
