 ...
 */
public class SaleCardPerformanceDataSource implements IAsyncDataSource<List<AmountItem>> {
    private PagedProxy proxy = new PagedProxy(20);
    private PageRequest request;

    public SaleCardPerformanceDataSource(PageRequest pageRequest) {
        super();
    ...
    private RequestHandle loadStores(final ResponseSender<List<AmountItem>> sender, final int page) throws Exception {
        ...
        final Subscription subscribe = User.get().isBoss()?getBossXKAmount(sender):getEmpXKAmount(sender);
    ...
    private Subscription getBossXKAmount(final ResponseSender<List<AmountItem>> sender) {
        return ApiFactory.getXKAmountList(request).subscribe(new Action1<ApiResponse<AmountListResponse>>() {
            @Override
    ...
    private Subscription getEmpXKAmount(final ResponseSender<List<AmountItem>> sender) {
        return ApiFactory.getEmpXKAmountList(request).subscribe(new Action1<ApiResponse<AmountListResponse>>() {
            @Override
            public void call(ApiResponse<AmountListResponse> response) {
                AmountListResponse data = response.getData();
                if (!proxy.isPageCountSet()) {
