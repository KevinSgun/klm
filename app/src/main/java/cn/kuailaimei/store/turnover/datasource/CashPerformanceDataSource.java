 ...
 */
public class CashPerformanceDataSource implements IAsyncDataSource<List<AmountItem>> {
    private PagedProxy proxy = new PagedProxy(20);
    private PageRequest request;

    public CashPerformanceDataSource(PageRequest pageRequest) {
        super();
    ...
    private RequestHandle loadStores(final ResponseSender<List<AmountItem>> sender, final int page) throws Exception {
        ...
        final Subscription subscribe = User.get().isBoss()?getBossCashPerformance(sender):getEmpCashPerformance(sender);
    ...
    private Subscription getEmpCashPerformance(final ResponseSender<List<AmountItem>> sender) {
        return ApiFactory.getEmpCashAmountList(request).subscribe(new Action1<ApiResponse<AmountListResponse>>() {
            @Override
    ...
    private Subscription getBossCashPerformance(final ResponseSender<List<AmountItem>> sender) {
        return ApiFactory.getCashAmountList(request).subscribe(new Action1<ApiResponse<AmountListResponse>>() {
            @Override
            public void call(ApiResponse<AmountListResponse> response) {
                AmountListResponse data = response.getData();
                if (!proxy.isPageCountSet()) {
